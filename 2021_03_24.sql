 ...
 . SELECT절에서도 사용가능 -> 스칼라 서브 쿼리 ( 스칼라 : 단일의)
    ...
    -- 버거지수 신희철
                    >SELECT, ORDER BY 절 서브쿼리
                         ...
                         - SELECT절(Scalar Subquery) : JOIN을 쓰는 것이 더 효율적일 수 있다.
                         - 성능 저하가 주의 된다.
                         - ORDER BY 절 --> 거의 쓰이지 않는다.
 . FROM절에서 사용 : 인라인 뷰
...
-- Single row Subquery  : 서브쿼리 결과가 단일 행   * 단일 행 비교 연산자에는 =, <, <=, > , >=, <>을 사용한다. 
-- Multiple row Subquery  : 서브쿼리 결과가 여러 행    * 다중 행 비교 연산자에는 IN, ALL, ANY, SOME, EXISTS가 있다.
                            IN 결과중 하나라도 일치하면 참
                            ANY,SOME 결과와 하나 이상 일치하면 참
                            ALL 결과와 모든 값이 일치하면 참
                            EXISTS 결과 중에서 값이 있으면 참 메인쿼리 수행, 없으면 거짓으로 메인쿼리 수행안함
-- Multiple column Subquery : 서브쿼리 결과가 여러 컬럼    * 메인쿼리의 조건절에 여러 컬럼을 동시에 비교가 가능하다.
